...
import inspect
from common.compiler.util import Production, Symbol, Entry
...
def P81():
    ...
    e = f.children[2]
    print(inspect.getmembers(e), type(e))
    code_output('IF ' + str(e.attr.get('value', "")) + ' GOTO ' + str(CODE_SIZE + 2))
    code_output(None)
...
def main():
    prepare_grammar()
    lexer2.read_source_file('input.c')
    do_parsing()
    print("SYMBOL TABLE")
    print("------------")
    print_symbol_table()
...
def server_main_sema(input_str):
    global TERMINAL_SET, NON_TERMINAL_SET, SYMBOL_DICT, PRODUCTION_LIST, PARSING_TABLE, SEMA_ACTION_TABLE, SYMBOL_STACK
    global SYMBOL_TABLE, LAST_STACK_TOP_SYMBOL, CODE_SIZE, CODE_RESULT, current_symbol_table_pos, current_symbol_index, CURRENT_CONDITION_NODE
    ...
    for r in CODE_RESULT:
        output_lines.append(str(CODE_RESULT.index(r)) + ': ' + r)
    return output_lines


if __name__ == '__main__':
    print(server_main_sema(input_str="""int main() {
    int a;
    a=1;
    if(a>2){
    int b;
